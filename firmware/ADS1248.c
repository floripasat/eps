...
void config_ADS1248(int positive_channel)
{
	...
	{
		UCB1TXBUF = initialization_data[w];
...
long read_ADS1248(int channel)
{
	...
	for (i = 500; i; i--);              // Add time between transmissions to make sure slave can keep up
	temp = (long)UCB1RXBUF << 16;				// save output to variable

	UCB1TXBUF = NOP_command;			// send dummy command to send clock and read conversion results
	for (i = 500; i; i--);              // Add time between transmissions to make sure slave can keep up
	temp |= (long)UCB1RXBUF << 8;				// save output to variable

	UCB1TXBUF = NOP_command;			// send dummy command to send clock and read conversion results
	for (i = 500; i; i--);              // Add time between transmissions to make sure slave can keep up
	temp |= UCB1RXBUF;				// save output to variable
