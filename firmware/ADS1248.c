...
void config_ADS1248(int channel)
{
	...
	P5OUT &= ~BIT4;		// pull chip select low to start communication
	...
	if(w == 21)
	{
		P5OUT |= BIT4;							// pull chip select high after communication is done
	}
}
void read_ADS1248(int channel)
{
	...
	P5OUT &= ~BIT4;						// pull chips select low to start communication
	__delay_cycles(7);					// delay of 7 tclk
	...
	__delay_cycles(7);					// delay of 7 tclk
	P5OUT |= BIT4;						// pull chip select high after communcation is done
